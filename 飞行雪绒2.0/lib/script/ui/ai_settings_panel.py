...
class AISettingsPanel(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        # 主题选择
        from config.config_ui import get_theme_name, set_theme as _set_theme, get_available_themes
        self._theme_combo = _WatermarkComboBox()
        self._theme_combo.setView(QListView(self._theme_combo))
        for theme_key in get_available_themes():
            display_name = '默认主题 (黑青粉)' if theme_key == 'default' else '粉白交替主题'
            self._theme_combo.addItem(display_name, theme_key)
        ...
        self._set_form_row_description(
            form,
            self._theme_combo,
            "切换界面配色方案；粉白主题以粉色和白色交替搭配，默认主题为黑青粉配色。",
        )
    ...
    def _on_theme_combo_changed(self, index: int) -> None:
        ...
        if theme_key:
            from config.config_ui import set_theme as _set_theme
            if _set_theme(theme_key):
                from lib.core.event.center import get_event_center, EventType, Event
                get_event_center().publish(Event(EventType.UI_THEME_CHANGED, {
                    'theme': theme_key,
                }))
