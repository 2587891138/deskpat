...
class CmdWindow(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._event_center.subscribe(EventType.UI_THEME_CHANGED, self._on_theme_changed)
    ...
    @staticmethod
    def _rewrite_claude_command(cmd: str) -> str:
        """将无 TTY 环境的 claude 交互模式自动转换为 -p 单次问答模式"""
        stripped = cmd.strip()
        if not stripped:
            return cmd
        # 匹配以 claude 开头的命令（排除 claude -p / claude --print / claude - 等情况）
        parts = stripped.split(None, 1)
        if parts[0] == 'claude':
            if len(parts) == 1:
                # 只输入了 claude → 转为 claude -p（会提示输入）
                return 'claude -p'
            rest = parts[1]
            # 如果已有 -p 或 --print 或 - 开头，保留原样
            if rest.startswith('-'):
                return cmd
            # 否则把后面的内容当作 prompt
    ...
    def _start_stream(self, cmd: str):
        ...
        # Claude CLI 智能路由：检测无 TTY 交互模式并自动转换为 -p 模式
        cmd = self._rewrite_claude_command(cmd)
